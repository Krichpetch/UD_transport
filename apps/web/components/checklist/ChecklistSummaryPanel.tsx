            ...
            {categoryBuckets.map((cat) => {
              ...
              return (
                <div key={cat.value} className="rounded-lg bg-secondary/40 p-2.5 text-center">
                  <p className="text-xs font-semibold text-muted-foreground">{cat.value}</p>
              ...
              <div key={cat.value}>
                ...
                {cat.groups.length > 1 && (
                  <div className="mt-1 space-y-1 pl-2">
                    {cat.groups.map((g) => (
                      <div key={g.groupId} className={`flex items-center justify-between ${comfortable ? 'text-sm' : 'text-xs'}`}>
