        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 sm:items-center" onClick={() => setOpen(false)}>
          ...
          >
            ...
            <div className="mt-0.5 flex items-center gap-1.5">
              <span className="shrink-0 rounded bg-secondary px-1.5 py-0.5 font-mono text-xs text-muted-foreground">{node.code}</span>
            ...
            {node.guidance && (
              <div className="mt-3 border-t border-border pt-3 text-xs text-gray-600">
                ...
                {node.guidance.reference && (
                  <p className="mt-1 text-xs text-muted-foreground">อ้างอิง: {node.guidance.reference}</p>
                )}
              </div>
            )}
          </div>
