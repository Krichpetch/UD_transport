      ...
      return (
        <div key={n.code}>
          <div className="flex items-center gap-1">
            ...
            >
              <span className="bg-secondary text-muted-foreground shrink-0 rounded px-1.5 py-0.5 font-mono text-xs">{n.code}</span>
          ...
          >
            <div className="min-w-0">
              <p className="text-foreground truncate text-sm">
                {p.code && (
                  <span className="bg-secondary text-muted-foreground mr-1.5 rounded px-1.5 py-0.5 font-mono text-xs">{p.code}</span>
                )}
          ...
          <div className="themed-scrollbar min-h-0 flex-1 overflow-y-auto px-4 pb-4 pt-1">
            {needle ? (
              searchResults.length > 0 ? (
                ...
                <p className="text-muted-foreground py-8 text-center text-base">ไม่พบรายการที่ค้นหา</p>
              )
            ) : grouped ? (
              sections.map((s) => {
