...
export function PhotoPicker({ onPhotosUploaded, disabled = false, existingCount = 0 }: Props) {
  ...
  if (!open) {
    return (
      <div className="mt-2.5 space-y-1">
        ...
        {atLimit && (
          <p className="text-xs text-amber-600">ครบจำนวนรูปภาพสูงสุด ({MAX_PHOTOS_PER_ITEM} รูปต่อรายการ) แล้ว</p>
        )}
      </div>
    )
  }
  ...
  return (
    <div className="mt-2.5 overflow-hidden rounded-xl border border-border bg-white shadow-sm">
          ...
          <div key={photo.id} className="relative">
            ...
            {photo.status === 'failed' && (
              <div className="absolute inset-0 flex items-center justify-center rounded-lg bg-red-500/20">
                <span className="px-0.5 text-center text-xs font-bold leading-tight text-red-700">ล้มเหลว</span>
              </div>
            )}
      ...
      {limitMessage && (
        <p className="px-3 pb-2 text-xs text-amber-600">{limitMessage}</p>
      )}
