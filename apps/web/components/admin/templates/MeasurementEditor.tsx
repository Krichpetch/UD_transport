...
export function TierRowsEditor({ tiers, onChange }: { tiers: TemplateTier[]; onChange: (tiers: TemplateTier[]) => void }) {
  ...
  return (
    <div className="space-y-1.5">
      <div className="text-muted-foreground grid grid-cols-[1fr_1fr_1fr_1fr_1fr_auto] gap-1 text-xs">
        <span>min</span>
        <span>max</span>
        <span>required</span>
        <span>+ต่อ</span>
      ...
      {readOnly ? (
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2">
            ...
            {editMeasurement.isError && <span className="text-xs text-red-500">{(editMeasurement.error as Error).message}</span>}
          </div>
