...
export function LabelByLawEditor({ templateId, node, readOnly }: { templateId: string; node: TemplateNode; readOnly: boolean }) {
  ...
  return (
    <div className="border-border bg-card space-y-2 rounded-xl border p-3">
      <p className="text-foreground text-base font-semibold">ข้อความตามยุคกฎหมาย (labelByLaw)</p>
      ...
      {existingCodes.length === 0 && <p className="text-muted-foreground text-xs">ยังไม่มีข้อความตามยุคกฎหมายสำหรับรายการนี้</p>}
      ...
      {editLabelByLaw.isError && <p className="text-xs text-red-500">{(editLabelByLaw.error as Error).message}</p>}
    </div>
  )
}
