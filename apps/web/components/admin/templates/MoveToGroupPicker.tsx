...
export function MoveToGroupPicker({
  ...
  return (
    <div className="border-border bg-secondary/30 space-y-1.5 rounded-lg border p-2">
      <p className="text-muted-foreground text-xs">เลือกกลุ่มปลายทาง (ค้นหาได้ทุกกลุ่ม ไม่จำกัดเฉพาะรหัสเดียวกัน)</p>
      {groupsQuery.isLoading && <p className="text-muted-foreground text-xs">กำลังโหลด…</p>}
      ...
      {!groupsQuery.isLoading && candidates.length === 0 && (
        <p className="text-muted-foreground text-xs">{allCandidates.length === 0 ? 'ไม่พบกลุ่มปลายทางอื่น' : 'ไม่พบกลุ่มที่ตรงกับคำค้นหา'}</p>
      )}
      {candidates.length > 0 && (
        <div className="flex items-center gap-1.5">
          <select className={`${SELECT_CLS} py-1 text-xs`} value={targetItemId} onChange={(e) => setTargetItemId(e.target.value)}>
          ...
          <button
            ...
            className="bg-primary text-primary-foreground flex shrink-0 items-center gap-1 rounded px-2 py-1 text-xs font-medium disabled:opacity-50"
          >
      ...
      <button type="button" onClick={() => setOpen(false)} className="text-muted-foreground text-xs hover:underline">
        ยกเลิก
      </button>
      {attachNodeToGroup.isError && <p className="text-xs text-red-500">{(attachNodeToGroup.error as Error).message}</p>}
    </div>
  )
}
