...
function LabelEditor({ templateId, node }: { templateId: string; node: TemplateNode }) {
  ...
  return (
    <div className="space-y-1.5">
      <p className="text-muted-foreground text-xs">ข้อความคำถาม (รหัสคงเดิม: <span className="font-mono">{node.code}</span>)</p>
      ...
      <div className="flex items-center gap-2">
        ...
        {editLabel.isError && <span className="text-xs text-red-500">{(editLabel.error as Error).message}</span>}
      </div>
    </div>
  )
}
...
function NewMeasurementForm({ templateId, nodeCode, onDone }: { templateId: string; nodeCode: string; onDone: () => void }) {
  ...
  return (
    <div className="border-border bg-secondary/30 space-y-2 rounded-lg border p-2.5">
      ...
      {operator === 'tiered' && (
        <p className="text-muted-foreground text-xs">จะสร้างตารางขั้นบันไดเปล่า 1 แถว — ปรับรายละเอียดได้จากตัวแก้ไขเกณฑ์หลังบันทึก</p>
      )}
      ...
      {addMeasurement.isError && <p className="text-xs text-red-500">{(addMeasurement.error as Error).message}</p>}
    </div>
  )
}
...
function AddChildForm({ templateId, parentCode, onDone }: { templateId: string; parentCode: string; onDone: (code: string) => void }) {
  ...
  return (
    <div className="border-border bg-secondary/30 space-y-2 rounded-lg border p-2.5">
      ...
      <p className="text-muted-foreground text-xs">รหัสจะถูกกำหนดโดยระบบอัตโนมัติ (ไม่ใช้ซ้ำแม้ลบไปแล้ว)</p>
      <div className="flex items-center gap-2">
        <button
          type="button"
          onClick={save}
      ...
      {addChild.isError && <p className="text-xs text-red-500">{(addChild.error as Error).message}</p>}
    </div>
  )
}
...
export function StructuralEditor({
  ...
  return (
    <div className="border-border bg-card space-y-3 rounded-xl border p-3">
      <p className="text-foreground text-base font-semibold">แก้ไขโครงสร้าง (เวอร์ชันร่าง)</p>

      <LabelEditor templateId={templateId} node={node} />

      {/* Part C.2 — question type selector */}
      <div>
        <p className="text-muted-foreground mb-1 text-xs">ประเภทคำถาม</p>
        ...
        {confirmingDowngradeTo && (
          <div className="mt-2 flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 p-2 text-xs text-amber-700">
      ...
      {(type === 'measured' || type === 'presence_standard') && (
        <div>
          <div className="mb-1 flex items-center justify-between">
            <p className="text-muted-foreground text-xs">เกณฑ์วัดค่า</p>
            {!addingMeasurement && (
              <button type="button" onClick={() => setAddingMeasurement(true)} className="text-accent flex items-center gap-1 text-xs hover:underline">
      ...
      <div className="border-border space-y-2 border-t pt-2">
        <div className="flex items-center justify-between">
          <p className="text-muted-foreground text-xs">รายการย่อย</p>
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={() =>
            ...
            {!addingChild && (
              <button type="button" onClick={() => setAddingChild(true)} className="text-accent flex items-center gap-1 text-xs hover:underline">
