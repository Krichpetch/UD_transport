...
import {
  useStationSummary, useStationMetrics, useStationMapNodes,
...
import { ChecklistItemPieChart } from '@/components/charts/ChecklistItemPieChart'
...
import { isWithinTimeframe, resolveTimeframeRange } from '@/lib/timeframe'
...
const DASHBOARD_TABS = [
  ...
  { key: 'issues',   label: 'ประเด็นที่ควรปรับปรุง' },
] as const
type DashboardTab = (typeof DASHBOARD_TABS)[number]['key']

export default function DashboardPage() {
  const { data: summary } = useStationSummary()
  const { data: mapNodes, isLoading: mapNodesLoading } = useStationMapNodes()
  const stations: Station[] = mapNodes ?? []

  // UDT-16 — the timeframe control itself lives in AppNavbar (top of the dashboard-layout page),
  // shared via this store since the navbar and this page are siblings under the layout.
  const { timeframe, customFrom, customTo, resetTimeframe } = useDashboardFiltersStore()
  const timeframeRange = React.useMemo(
  ...
  React.useEffect(() => { setTablePage(1) }, [modeFilter, regionFilter, provinceFilter, agencyFilter, timeframe, customFrom, customTo, cabinetOnly])
  ...
  const hasFilters = !!(
    modeFilter || regionFilter || provinceFilter || agencyFilter || categoryFilter || subItemFilter ||
    timeframe !== 'all' || cabinetOnly
  )
  ...
  function clearFilters() {
    ...
    setSubItemFilter('')
    resetTimeframe()
    setCabinetOnly(false)
  }

  // UDT-18 — station ids that "ผ่านมติ ครม.", scoped to the rest of the active filters (excluding
  // cabinetOnly itself, which just gates whether this fetch runs at all).
  const baseScopeFilters = {
    mode:              modeFilter,
    region:            regionFilter,
  ...
  const cabinetIdsQuery = useCabinetApprovedIds(baseScopeFilters, cabinetOnly)
  const cabinetIds = cabinetIdsQuery.data
  ...
  const filteredStations = stations.filter(s =>
    ...
    isWithinTimeframe(s.lastInspected, timeframeRange) &&
    (!cabinetOnly     || (cabinetIds?.includes(s.id) ?? false))
  )
  ...
  const metricsQuery = useStationMetrics(
    {
      ...
      cabinetApproved: cabinetOnly || undefined,
    },
    !!subItemFilter,
  )
  ...
  // UDT-17 — "ประเด็นที่ควรปรับปรุง" (groups + items), scoped to the same shared filters as the
  // overview tab. Only fetched while its own tab is open — no point aggregating it in the
  // background while the user is looking at the overview.
  const issueSummaryQuery = useIssueSummary({
    ...baseScopeFilters,
    cabinetApproved: cabinetOnly || undefined,
  })
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Tab strip — overview (KPIs/chart/map/table, unchanged) vs. ประเด็นที่ควรปรับปรุง (tucked
          away per user request). Same button+underline idiom AppNavbar's own notification tabs
          use, kept bespoke rather than a new shadcn primitive. */}
        ...
        <>
              ...
              {metricsLoading ? (
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                  ...
                  <div className="border-border rounded-lg border p-3">
                    ...
                    <div className="h-28">
                      <ChecklistItemPieChart histogram={metrics.histogram} showLegend />
                    </div>
                  </div>
                </div>
              ) : (
          ...
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
            <div className="bg-card border-border rounded-xl border p-5">
              ...
              <p className="text-foreground text-3xl font-bold">{mapNodesLoading ? '…' : kpi.totalStations.toLocaleString()}</p>
              <p className="text-muted-foreground mt-1 text-xs">{hasFilters ? 'ตามตัวกรองที่เลือก' : 'ครอบคลุมทุกประเภท'}</p>
            </div>
      ...
      {activeTab === 'issues' && (
        <IssueSummaryPanel
          groups={issueSummaryQuery.data?.groups}
          items={issueSummaryQuery.data?.items}
          loading={issueSummaryQuery.isLoading}
        />
      )}
    </div>
  )
